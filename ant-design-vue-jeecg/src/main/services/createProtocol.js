...
export default (scheme, serverPath = __dirname) =>
  protocol.registerBufferProtocol(scheme, (req, next) => {
    ...
    console.log('file ', path.join(serverPath, reqPath));
    fs.readFile(path.join(serverPath, reqPath), (err, data) => {
      ...
      if (!err) {
        ...
        console.error(
          `Failed to read ${reqPath} on ${scheme} protocol`,
          err
        )
        next({
          mimeType: null,
